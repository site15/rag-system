...
import { ChatDocumentEmbedding } from '../generated/prisma/client';
...
export class ChatDocumentEmbeddingDumpService {
   ...
   */
  static async createFullDump(): Promise<ChatDocumentEmbeddingDumpResponse> {
    try {
      Logger.logInfo('Creating full dump of ChatDocumentEmbedding table');

      // Fetch all records from ChatDocumentEmbedding table
      const records = await PrismaService.instance.$queryRawUnsafe<
...
order by "createdAt" asc`);
      ...
      const insertStatements = records.map((record) =>
        ChatDocumentEmbeddingDumpService.generateInsertStatement(record),
      );
   ...
   */
  private static generateInsertStatement(record: any): string {
