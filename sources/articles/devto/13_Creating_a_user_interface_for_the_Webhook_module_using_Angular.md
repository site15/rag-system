...
Published: 2024-10-25T20:01:49.890Z
Tags: angular, webhook, fullstack, nestjs
[Original Article](https://dev.to/endykaufman/creating-a-user-interface-for-the-webhook-module-using-angular-40id)
...
- Fill in the user data - substitutes the `xExternalUserId` and `xExternalTenantId` with the hidden random `uuid` identifiers;

- Fill in the administrator data - inserts the user ID with the role `Admin` into the `xExternalUserId`, the backend creates this user at startup, and the ID is inserted into the frontend when it is assembled into `CI\CD`.
