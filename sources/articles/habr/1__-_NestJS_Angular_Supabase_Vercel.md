...
Дата публикации: Fri, 14 Feb 2025 18:47:02 GMT
[Оригинал статьи](https://habr.com/ru/articles/882726/?utm_campaign=882726&amp;utm_source=habrahabr&amp;utm_medium=rss)
...
[Читать далее](https://habr.com/ru/articles/882726/?utm_campaign=882726&amp;utm_source=habrahabr&amp;utm_medium=rss#habracut)

[](/ru/users/kaufmanendy/)[kaufmanendy](/ru/users/kaufmanendy/)14  фев   в 18:47# Как я развернул фулстек-приложение на «NestJS» с «Angular» в «Supabase» и «Vercel»

Время на прочтение6 минОхват и читатели1.8K[Angular * ](/ru/hubs/angular/)[NestJS * ](/ru/hubs/nestjs/)[DevOps * ](/ru/hubs/devops/)[Тестирование веб-сервисов * ](/ru/hubs/web_testing/)РетроспективаПредыдущая статья: Конвертация даты по временной зоне пользователя в "NestJS", а также ввод и отображение даты в "Angular"
...
- Для миграций использован `pg-flyway` (мини-версия `Flyway` без `Java`). Я не хотел отказываться от функционала `Flyway`, но при этом не хотел ставить `Java` в момент деплоя приложения. В итоге я написал миниверсию `flyway` — `pg-flyway`.
...
- В процессе поиска я увидел, что `CacheModule` для `NestJS` переходит на использование `Keyv`, и я написал свою обертку `nestjs-mod/keyv`, которая поддерживает как `Redis`, так и `PostgreSQL`.
...
- Написан новый `NestJS`-модуль для работы с `Supabase Auth`, совместимый с предыдущей реализацией. Этот модуль был написан не с нуля, а путем копирования кода существующего `nestjs-mod/authorizer`.
...
- Эта штука сожрала очень много времени, я не буду описывать все проблемы на пути, но их было очень много. Просто закину ссылку на примеры конфигураций, жалко, что я увидел их только недавно: примеры конфигураций `Vercel`, а это мой конфиг для `vercel.json`.
...
- Не стану описывать регистрацию в сервисах `Supabase` и `Vercel`. Просто приложу небольшое видео о том, как создавать приложения на `Supabase` и прописать переменные окружения в `Vercel`.
...
- Создать `bucket` с названием "images" в хранилище (пример ссылки: https://supabase.com/dashboard/project/XXX/storage/buckets)

- Создайте новые "S3 Access Keys" с "Access key ID" и "Secret access key" (пример ссылки: https://supabase.com/dashboard/project/gustcjgbrmmipkizqzso/settings/storage)
...
Теги:- [angular](/ru/search/?target_type=posts&amp;order=relevance&amp;q=[angular])
- [nestjs](/ru/search/?target_type=posts&amp;order=relevance&amp;q=[nestjs])
- [nestjsmod](/ru/search/?target_type=posts&amp;order=relevance&amp;q=[nestjsmod])
- [supabase](/ru/search/?target_type=posts&amp;order=relevance&amp;q=[supabase])
- [vercel](/ru/search/?target_type=posts&amp;order=relevance&amp;q=[vercel])
- [fullstack](/ru/search/?target_type=posts&amp;order=relevance&amp;q=[fullstack])
- [ng](/ru/search/?target_type=posts&amp;order=relevance&amp;q=[ng])
...
- [devops](/ru/search/?target_type=posts&amp;order=relevance&amp;q=[devops])
