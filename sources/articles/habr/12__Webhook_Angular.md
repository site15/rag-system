...
Дата публикации: Fri, 25 Oct 2024 20:01:33 GMT
[Оригинал статьи](https://habr.com/ru/articles/853582/?utm_campaign=853582&amp;utm_source=habrahabr&amp;utm_medium=rss)

**Описание из RSS:**
![undefined](https://habrastorage.org/getpro/habr/upload_files/daa/9fe/a03/daa9fea0332563629db94e0277fa0472.png)Cоздание таблички отображающей данные и формы для ее заполнения, интерфейсы строятся на компонентах от https://ng.ant.design, формы создаются и управляются с помощью https://formly.dev, для стилей используется https://tailwindcss.com, стейт машины нет.

[Читать далее](https://habr.com/ru/articles/853582/?utm_campaign=853582&amp;utm_source=habrahabr&amp;utm_medium=rss#habracut)

[](/ru/users/kaufmanendy/)[kaufmanendy](/ru/users/kaufmanendy/)25  окт  2024 в 20:01# Создание пользовательского интерфейса для модуля Webhook с помощью Angular

Время на прочтение26 минОхват и читатели744[Angular * ](/ru/hubs/angular/)[NestJS * ](/ru/hubs/nestjs/)[TypeScript * ](/ru/hubs/typescript/)[Тестирование веб-сервисов * ](/ru/hubs/web_testing/)[Дизайн](/ru/hubs/design/)ТуториалПредыдущая статья: Создание конфигурируемого Webhook-модуля для NestJS-приложении
...
- Заполнить данные пользователя - подставляет в `xExternalUserId` и `xExternalTenantId` захардкоженные случайные `uuid`-идентификаторы;

- Заполнить данные администратора - подставляет в `xExternalUserId` идентификатор пользователя с ролью `Admin`, бэкенд при старте создает этого пользователя, а идентификатор вставляется во фронтенд при сборке его в `CI\CD`.
